...
function syncUI(){
 ...
 let s=document.getElementById('q20Status');if(s){s.id='q21Status';s.innerHTML='<b>SIGNAL 021</b> · LMB MOVE · HOLD Q SIGNAL · HOLD W PRISM · HOLD E RESONANCE · HOLD R FRACTURE · SPACE PHASE'}
 s=document.getElementById('q21Status');if(s)s.innerHTML='<b>SIGNAL 021</b> · LMB MOVE · HOLD Q SIGNAL · HOLD W PRISM · HOLD E RESONANCE · HOLD R FRACTURE · SPACE PHASE';
 const mark=document.getElementById('q20Mark');if(mark){mark.id='q21Mark';mark.innerHTML='<b>SIGNAL 021</b> · QWER HOLD-CAST · DRUID + PUNKIN'}
 const zone=document.querySelector('.miniwrap .sub');if(zone&&/SIGNAL\s+\d+/.test(zone.textContent||''))zone.textContent='SIGNAL 021';
...
const obs=new MutationObserver(syncUI);obs.observe(document.documentElement,{childList:true,subtree:true});syncUI();setTimeout(syncUI,500);setTimeout(syncUI,1800);
function loop(now){let dt=Math.min(.05,Math.max(.001,(now-last)/1000));last=now;if(dt>0){if(held.q)cast('q');if(held.w)cast('w');if(held.e)cast('e');if(held.r)cast('r')}requestAnimationFrame(loop)}
