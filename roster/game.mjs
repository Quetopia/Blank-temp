...
// Original channel artwork is used for selection; gameplay keeps its own render assets.
const sourceArtIds=new Set(['diamond-girl','crescent-weaver','ember-sovereign','sally','cyberdine','suneye','reef-lobster']);
function selectionArt(id){const source=id==='abyssal-cantor'?'reef-lobster':id;return sourceArtIds.has(source)?`./source-art/${source}-original.jpg`:art(id);}
...
for(const c of ROSTER){const b=document.createElement('button');b.dataset.character=c.id;b.innerHTML=`<img src="${selectionArt(c.id)}" alt=""><strong>${c.name}</strong><small>& ${c.pet}</small>`;b.onclick=()=>choose(c.id,selectedBuild);$('#rosterGrid').append(b);}
async function choose(id,build){const token=++selectionToken;selectedId=id;selectedBuild=build;game.select(id,build);const c=game.spec;document.documentElement.style.setProperty('--accent',c.color);$('#heroArt').src=selectionArt(id);$('#heroArt').alt=id==='abyssal-cantor'?'Reef Lobster, the companion and ocean inspiration':c.name;$('#artCaption').textContent=id==='abyssal-cantor'?'Reef Lobster · original companion artwork':id==='druid'?'Mushroom Druid · original artwork':'Original artwork from @quetopia3';$('#heroTitle').textContent=c.title;$('#heroName').textContent=c.name;$('#heroRole').textContent=c.role;$('#heroStory').textContent=c.story;$('#heroPet').textContent=`Companion: ${c.pet} · “${c.promise}”`;$('#heroSkills').innerHTML=c.skills.map((s,i)=>`<p><b>${['Q','W','E','R'][i]} · ${s.name}</b>${s.description}<br><small>${s.cd}s cooldown${s.cost?` · ${s.cost} spirit`:""}</small></p>`).join('');$('#builds').innerHTML='';c.builds.forEach((name,i)=>{const b=document.createElement('button');b.textContent=name;b.setAttribute('aria-pressed',i===build);b.onclick=()=>choose(id,i);$('#builds').append(b);});$('#buildDescription').textContent=['Starter focus: +8% damage to all attacks.',id==='ember-sovereign'?'Starter focus: +25% claw and breath damage.':'Starter focus: +25% projectile damage.','Starter focus: +30 life, faster companion support, extra companion strike; −8% damage.'][build];for(const b of document.querySelectorAll('[data-character]'))b.setAttribute('aria-pressed',b.dataset.character===id);for(const [key,b] of buttonMap){b.querySelector('small').textContent=game.kit[key].name;b.title=game.kit[key].description||game.kit[key].name;}$('.skillCaption').firstChild.textContent=c.name.toUpperCase()+' ';$('.companion strong').textContent=c.pet.toUpperCase();$('.companion span').textContent='Unwritten bond';$('.companion img').src=selectionArt(c.petId);$('.companion img').alt=c.pet;for(const key of ['nova','roots','heal','cat','cast'])colors[key]=c.color;
...
const campaignUI=setupCampaign({game,start,art:selectionArt,clearEffects:()=>{fx=[];numbers=[];ghosts=[];lastCat={x:game.cat.x,y:game.cat.y};}});
updateProgress();
try{[ground,atlas]=await Promise.all([loadImage('../grove/assets/grove.webp'),loadImage('../grove/assets/characters.webp')]);spriteCells(atlas);import('../grove/punkin-3d.mjs').then(m=>m.createPunkinRenderer()).then(r=>{punkin3d=r;}).catch(error=>console.warn('Punkin renderer unavailable',error));
 ...
 $('.disclosure').textContent='Starter roster playtest · rough 3D studies · advanced skill trees still in development';loaded=true;$('#start').disabled=false;$('#start').textContent='Choose a chapter';$('#hud').style.opacity='.35';requestAnimationFrame(frame);choose(selectedId,selectedBuild);}catch(e){$('#start').textContent='Artwork failed to load';$('.instructions').textContent='Reload this page to retry. '+e.message;console.error(e);}
