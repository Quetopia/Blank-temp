...
export class Grove {
 ...
 if(id==='dash'){this.pendingBolt=null;this.wandFlash=0;let dx=aim.x-p.x,dy=aim.y-p.y,d=Math.hypot(dx,dy)||1;const to=constrain({x:p.x+dx/d*155,y:p.y+dy/d*155});this.dash={from:{x:p.x,y:p.y},to,elapsed:0,duration:.28};p.inv=.6;this.target=null;this.event('dash',p,{life:.28});}
 return true;}
 ...
 if(this.dash){const dash=this.dash;dash.elapsed=Math.min(dash.duration,dash.elapsed+dt);const t=dash.elapsed/dash.duration,ease=1-(1-t)**3;p.x=dash.from.x+(dash.to.x-dash.from.x)*ease;p.y=dash.from.y+(dash.to.y-dash.from.y)*ease;this.target=null;if(t>=1){this.dash=null;this.event('dashEnd',p,{life:.35});}}
