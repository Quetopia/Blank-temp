...
export class Grove {
 ...
 constrain(p){return constrain(p);}
 ...
 move(p,to,speed,dt){const d=Math.hypot(to.x-p.x,to.y-p.y);if(d<3)return false;const k=Math.min(1,speed*dt/d);this.aimDir(p,to);p.x+=(to.x-p.x)*k;p.y+=(to.y-p.y)*k;p.walk+=speed*dt*.055;this.constrain(p);return true;}
 ...
 cast(id,aim){if(this.mode!=='playing'||this.dash)return false;const s=SKILLS[id],p=this.player;if(!s||(id==='bolt'&&this.pendingBolt)||this.cd[id]>0||p.mana<s.cost)return false;this.cd[id]=s.cd;p.mana-=s.cost;p.cast=.22;this.aimDir(p,aim);if(id==='bolt'){
  this.pendingBolt={aim:{x:aim.x,y:aim.y},elapsed:0};p.cast=WAND_CHARGE+.14;}
 ...
 if(id==='dash'){this.pendingBolt=null;this.wandFlash=0;let dx=aim.x-p.x,dy=aim.y-p.y,d=Math.hypot(dx,dy)||1;const to=this.constrain({x:p.x+dx/d*155,y:p.y+dy/d*155});this.dash={from:{x:p.x,y:p.y},to,elapsed:0,duration:.28};p.inv=.6;this.target=null;this.event('dash',p,{life:.28});}
 return true;}
 releaseBolt(from=this.muzzle||{x:this.player.x+32,y:this.player.y-125}){
 const pending=this.pendingBolt;
 if(!pending||pending.elapsed+1e-8<WAND_CHARGE||this.mode!=='playing'||this.dash)return false;
 if(!Number.isFinite(from?.x)||!Number.isFinite(from?.y))return false;
 const dx=pending.aim.x-from.x,dy=pending.aim.y-from.y,d=Math.hypot(dx,dy)||1;
 this.shots.push({x:from.x,y:from.y,vx:dx/d*640,vy:dy/d*640,life:1.45,power:34*this.power,pierce:this.blessing==='spores'?1:0,hit:new Set()});
 this.pendingBolt=null;this.wandFlash=.12;this.event('cast',from,{r:38,life:.18});return true;
 }
 ...
 step(dt,input={}){if(this.mode!=='playing')return;this.time+=dt;this.wandFlash=Math.max(0,this.wandFlash-dt);if(this.pendingBolt)this.pendingBolt.elapsed+=dt;let p=this.player;p.inv=Math.max(0,p.inv-dt);p.cast=Math.max(0,p.cast-dt);p.mana=Math.min(100,p.mana+12*dt);for(const k in this.cd)this.cd[k]=Math.max(0,this.cd[k]-dt);
 if(this.dash){const dash=this.dash;dash.elapsed=Math.min(dash.duration,dash.elapsed+dt);const t=dash.elapsed/dash.duration,ease=1-(1-t)**3;p.x=dash.from.x+(dash.to.x-dash.from.x)*ease;p.y=dash.from.y+(dash.to.y-dash.from.y)*ease;this.constrain(p);this.target=null;if(t>=1){this.dash=null;this.event('dashEnd',p,{life:.35});}}
 else if(input.direction&&(input.direction.x||input.direction.y)){this.target=null;this.move(p,{x:p.x+input.direction.x*100,y:p.y+input.direction.y*100},220,dt);}
 else {if(input.move)this.target=this.constrain({...input.move});if(this.target){if(!this.move(p,this.target,220,dt))this.target=null;}}
 if(input.fire&&input.aim)this.cast('bolt',input.aim);
 if(!this.wandDriven)this.releaseBolt();
 for(const e of this.enemies){if(e.dead)continue;e.root=Math.max(0,e.root-dt);e.flash=Math.max(0,e.flash-dt);e.cool-=dt;let d=distance(p,e);if(this.world&&d>900&&!e.wind)continue;if(e.wind>0){e.wind-=dt;if(e.wind<=0){this.event('strike',e.mark,{r:e.boss?140:65,boss:e.boss});if(distance(p,e.mark)<(e.boss?140:65))this.hurt(e.boss?45:18);if(this.mode!=='playing')return;e.cool=e.boss?1.3:1.5;}}else if(e.cool<=0&&d<(e.boss?330:95)){e.mark={x:p.x,y:p.y};e.wind=e.boss?.95:.65;this.event('telegraph',e.mark,{r:e.boss?140:65,life:e.wind});}else if(!e.root&&d>55)this.move(e,p,e.speed,dt);}
