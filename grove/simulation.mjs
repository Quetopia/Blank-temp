...
export class Grove {
 constructor(seed=9){this.seed=seed;this.reset();}
 ...
 hurt(n){let p=this.player;if(p.inv>0)return;p.hp=Math.max(0,p.hp-n);p.inv=.4;this.event('hurt',p,{n});if(p.hp<=0){this.mode='lost';this.event('notice',p,{text:'THE ROOTS WILL BRING YOU BACK'});}}
 cast(id,aim){if(this.mode!=='playing')return false;const s=SKILLS[id],p=this.player;if(!s||this.cd[id]>0||p.mana<s.cost)return false;this.cd[id]=s.cd;p.mana-=s.cost;p.cast=.22;this.aimDir(p,aim);if(id==='bolt'){let dx=aim.x-p.x,dy=aim.y-p.y,d=Math.hypot(dx,dy)||1;this.shots.push({x:p.x,y:p.y-20,vx:dx/d*640,vy:dy/d*640,life:1.45,power:34*this.power});this.event('cast',p);}
 if(id==='nova'){for(const e of this.enemies)if(distance(p,e)<245)this.damage(e,90*this.power);this.event('nova',p,{r:245});}
 if(id==='roots'){for(const e of this.enemies)if(distance(aim,e)<170){e.root=3;this.damage(e,40*this.power);}this.event('roots',aim,{r:170});}
 ...
 if(input.move)this.target=constrain({...input.move});if(this.target){if(!this.move(p,this.target,220,dt))this.target=null;}
 if(input.fire&&input.aim)this.cast('bolt',input.aim);
 for(const e of this.enemies){if(e.dead)continue;e.root=Math.max(0,e.root-dt);e.flash=Math.max(0,e.flash-dt);e.cool-=dt;let d=distance(p,e);if(e.wind>0){e.wind-=dt;if(e.wind<=0){this.event('strike',e.mark,{r:e.boss?140:65,boss:e.boss});if(distance(p,e.mark)<(e.boss?140:65))this.hurt(e.boss?45:18);e.cool=e.boss?1.3:1.5;}}else if(e.cool<=0&&d<(e.boss?330:95)){e.mark={x:p.x,y:p.y};e.wind=e.boss?.95:.65;this.event('telegraph',e.mark,{r:e.boss?140:65,life:e.wind});}else if(!e.root&&d>55)this.move(e,p,e.speed,dt);}
