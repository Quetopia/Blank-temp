...
export class Grove {
 ...
 cast(id,aim){if(this.mode!=='playing'||this.dash)return false;const s=SKILLS[id],p=this.player;if(!s||this.cd[id]>0||p.mana<s.cost)return false;this.cd[id]=s.cd;p.mana-=s.cost;p.cast=.22;this.aimDir(p,aim);if(id==='bolt'){let dx=aim.x-p.x,dy=aim.y-p.y,d=Math.hypot(dx,dy)||1;this.shots.push({x:p.x,y:p.y-20,vx:dx/d*640,vy:dy/d*640,life:1.45,power:34*this.power,pierce:this.blessing==='spores'?1:0,hit:new Set()});this.event('cast',p);}
 ...
 if(id==='dash'){let dx=aim.x-p.x,dy=aim.y-p.y,d=Math.hypot(dx,dy)||1;const to=constrain({x:p.x+dx/d*155,y:p.y+dy/d*155});this.dash={from:{x:p.x,y:p.y},to,elapsed:0,duration:.28};p.inv=.6;this.target=null;this.event('dash',p,{life:.28});}
 return true;}
