...
let punkin3d=null,previousPositions=new WeakMap();
...
function start(){previousPositions=new WeakMap();punkin3d?.reset(game.cat);clearInput();game.setBlessing(progress.blessing);game.start();setPause(false);winRecorded=false;$('#menu').hidden=true;$('#end').hidden=true;$('#hud').style.opacity=1;canvas.focus();last=performance.now();acc=0;}
$('#start').onclick=start;
...
updateProgress();
try{[ground,atlas]=await Promise.all([loadImage('assets/grove.webp'),loadImage('assets/characters.webp')]);spriteCells(atlas);$('#start').textContent='Preparing Punkin…';try{const {createPunkinRenderer}=await import('./punkin.bundle.mjs');punkin3d=await createPunkinRenderer();$('.disclosure').textContent='The Fractured Grove · 3D Punkin integration';}catch(error){console.error('Punkin 3D unavailable',error);$('.disclosure').textContent='3D unavailable · using original Punkin artwork';}loaded=true;$('#start').disabled=false;$('#start').textContent='Enter the grove';$('#hud').style.opacity='.35';requestAnimationFrame(frame);}catch(e){$('#start').textContent='Artwork failed to load';$('.instructions').textContent='Reload this page to retry. '+e.message;console.error(e);}
