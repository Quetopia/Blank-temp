...
const AUDIO_URL='https://raw.githubusercontent.com/Quetopia/Blank-temp/main/public/quetopia-reward-hit.mp3'
...
function playRewardSound(kind:Reward['kind']){
 try{
  ...
  activeAudio=a
  const p=a.play()
  if(p)p.catch(err=>console.error('Quetopia reward audio failed',err))
  setTimeout(()=>{if(activeAudio===a){a.pause();activeAudio=null}},kind==='supernova'?1500:1100)
 }catch(err){console.error('Quetopia reward audio exception',err)}
}
...
export default function RewardLayer(){
 ...
 useEffect(()=>{
  const warm=new Audio(AUDIO_URL);warm.preload='auto'
  const click=(e:MouseEvent)=>{const target=(e.target as HTMLElement).closest('button,.uploadButton') as HTMLElement|null;if(!target)return;const rect=target.getBoundingClientRect(),x=e.clientX||rect.left+rect.width/2,y=e.clientY||rect.top+rect.height/2,id=Date.now()+Math.random();setSparks(s=>[...s.slice(-5),{x,y,id}]);setTimeout(()=>setSparks(s=>s.filter(v=>v.id!==id)),720);const txt=(target.innerText||'').trim().toLowerCase();if(txt.includes('sync my ai')){playRewardSound('supernova');fire({title:'YOUR AI IS ALIVE',sub:'SIGNAL SYNCHRONIZED',kind:'supernova',id:Date.now()},true)}}
  document.addEventListener('click',click,true)
  const observer=new MutationObserver(muts=>{for(const m of muts){for(const node of Array.from(m.addedNodes)){if(!(node instanceof HTMLElement))continue;const candidates=[node,...Array.from(node.querySelectorAll?.('.toast,.notice')||[]) as HTMLElement[]];for(const c of candidates){if(!c.matches?.('.toast,.notice')||c.classList.contains('bad'))continue;const r=rewardForText(c.textContent||'');if(r&&r.kind!=='supernova'){setTimeout(()=>fire(r),100);return}}}}});observer.observe(document.body,{childList:true,subtree:true});return()=>{document.removeEventListener('click',click,true);observer.disconnect();if(activeAudio){activeAudio.pause();activeAudio=null}}},[])
